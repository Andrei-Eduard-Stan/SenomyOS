...
# Git preserves only the executable bit, so privileged destination modes must
# be asserted at the deployment manifest boundary rather than on a regular
# source checkout.
jq -e '
  [.components[] | select(.id == "recovery") | .entries[] |
    select(.source == "components/recovery/senomy-recovery.sudoers" and
           .target.root == "sudoers" and .mode == "0440")] | length == 1
' "$REPO_ROOT/deploy/system-manifest.json" >/dev/null

while IFS= read -r file; do
  case "$file" in
    *.png) expected=644 ;;
    *.sudoers) expected=644 ;;
