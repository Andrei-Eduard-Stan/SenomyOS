...
| Performance Dashboard | Expanded system-management surface | performance window, sections, scripts | `/proc`, `ps`, network/storage tools, allowlisted actions | Conditional polls under `active_surface == performance` | Later dashboard-only service tier | **Keep; do not migrate in Milestone 1** | High: expensive data, actions, safety confirmations |
...
| Network/Wi-Fi | Connection and signal state | `bar-network-listener.sh`, `network-status.sh`, Wi-Fi widget | NetworkManager via `nmcli` | long-running listener plus conditional polls | NetworkManager D-Bus service/model | Temporary honest placeholder; keep Eww | High: secrets, portals, device churn, no bundled native wrapper |
...
| Notifications | Bell state, flyout, retained history | SwayNC listener, notification scripts, notification/Insights widgets | `swaync-client`, local history | persistent listener + conditional polls | Native notification server/history policy | Keep on Eww/SwayNC | High: ownership, privacy, action tokens, DND |
| System tray | StatusNotifier items and flyout | `system-tray.yuck`, `tray-flyout.yuck`, background-app script | StatusNotifier inventory | Eww widget plus conditional 2 s poll | `Quickshell.Services.SystemTray` | Native item count now; keep menu/flyout on Eww | Medium: menus, activation geometry, watcher ownership |
...
| Power/session | Power profile, brightness, logout/reboot/shutdown | power flyout/control section and action scripts | UPower, brightness, systemd/logind | guarded polls and explicit confirmation state | logind/UPower action service with confirmation model | Disabled visual seam only in V2; keep Eww authoritative | Critical: destructive actions |
...
| Multi-monitor/layout profiles | Adapt to monitor width and density | `bar-layout.sh`, profile status, Eww screen choice | `hyprctl -j monitors`, profile JSON | 15 s layout poll + startup-selected screen | `Quickshell.screens`, per-screen variants, width breakpoints | Architectural foundation ported; not fully QA-tested | High: hotplug, scale, rotation, narrow/touch profiles |
| Startup/recovery | Start one stable shell and preserve fallback | Hyprland Lua, `start-eww.sh`, `senomy-shellctl.sh` | compositor startup and Eww IPC | detached daemon, readiness checks, main-bar open | future supervised Quickshell unit/health check | Keep Eww unchanged; V2 manual only | Critical: desktop availability and rollback |

## Shared state and service architecture

V2 separates compositor/system sources from presentation. Hyprland, UPower,
PipeWire, and System Tray are subscribed to once through Quickshell services.
...
V2 copies generated SenomyOS modules into its own shell directory because
Quickshell treats that directory as the portable configuration boundary.
`SenomyFrame.qml` retains corner and motif dimensions and stretches only the
neutral edges. The copied files remain generated artifacts; the source of truth
is the existing appearance frame system and generator.

Before wider migration, extend the generator to emit `Theme.qml` and refresh
the V2 frame copies so Eww, Rofi, Quickshell, boot, and recovery surfaces cannot
drift.
...
2. Rail renders correctly on the reference T480 and at least one narrow and one
